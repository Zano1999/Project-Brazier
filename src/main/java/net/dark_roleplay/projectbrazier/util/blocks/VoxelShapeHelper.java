	...
	public static VoxelShape rotateShape(VoxelShape shape, Direction direction) {
		...
		shape.forAllBoxes((x1, y1, z1, x2, y2, z2) -> {
			...
			if(direction == Direction.EAST)
				rotatedShapes.add(constructBoxSafe(8 - z1, y1 + 8, 8 + x1, 8 - z2, y2 + 8, 8 + x2));
			else if(direction == Direction.SOUTH)
				rotatedShapes.add(constructBoxSafe(8 - x1, y1 + 8, 8 - z1, 8 - x2, y2 + 8, 8 - z2));
			else if(direction == Direction.WEST)
				rotatedShapes.add(constructBoxSafe(8 + z1, y1 + 8, 8 - x1, 8 + z2, y2 + 8, 8 - x2));
			else if(direction == Direction.UP)
				rotatedShapes.add(constructBoxSafe(x1 + 8, 8 - z2, 8 + y1, x2 + 8, 8 - z1, 8 + y2));
			else if(direction == Direction.DOWN)
				rotatedShapes.add(constructBoxSafe(x1 + 8, 8 + z1, 8 - y2, x2 + 8, 8 + z2, 8 - y1));
