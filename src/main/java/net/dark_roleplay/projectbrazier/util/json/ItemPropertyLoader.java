...
import net.dark_roleplay.projectbrazier.handler.MedievalCreativeTabs;
...
public class ItemPropertyLoader {
	...
	public static Item.Properties properties(String name){
		...
		if(name != null) {
			try (JsonReader reader = new JsonReader(new InputStreamReader(ItemPropertyLoader.class.getClassLoader().getResourceAsStream("fixed_data/projectbrazier/properties/item/" + name + ".json")))) {
				reader.beginObject();
