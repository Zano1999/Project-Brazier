...
public class DecorItem extends Item {
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		capability.ifPresent(decorCon -> {
			DecorChunk decChunk = decorCon.getDecorChunk(context.getPos().getY()/16);
			DecorState decorState;
			decChunk.addDecor(decorState = new DecorState(decor));
			decorState.setPosition(new Vector3d(context.getPos().getX() & 0xF, Math.floorMod(context.getPos().getY(), 16), Math.floorMod(context.getPos().getZ(), 16)));
		});
