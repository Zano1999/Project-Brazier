package net.dark_roleplay.project_brazier.features.model_loaders.simple_pane_conneted_model;

import com.google.gson.JsonDeserializationContext;
import com.google.gson.JsonObject;
import net.dark_roleplay.project_brazier.features.model_loaders.axis_connected_models.AxisConnectionType;
...
public class SimplePaneConnectedModel implements IModelGeometry {
	...
	public static class ConnectedBakedModel extends BakedModelWrapper {
		...
		public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData) {
			boolean flag = state.has(BlockStateProperties.HORIZONTAL_FACING) && state.get(BlockStateProperties.HORIZONTAL_FACING).getAxisDirection() == Direction.AxisDirection.POSITIVE;
		...
		public IModelData getModelData(@Nonnull ILightReader world, @Nonnull BlockPos pos, @Nonnull BlockState state, @Nonnull IModelData tileData) {
			...
			if ((flag = state.has(BlockStateProperties.HORIZONTAL_AXIS)) || (flag2 = state.has(BlockStateProperties.AXIS)) || state.has(BlockStateProperties.HORIZONTAL_FACING)) {
				Direction.Axis axis = flag ?
						state.get(BlockStateProperties.HORIZONTAL_AXIS) :
						flag2 ?
								state.get(BlockStateProperties.AXIS) :
								state.get(BlockStateProperties.HORIZONTAL_FACING).getAxis();
